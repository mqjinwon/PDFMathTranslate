...
|----------------------|----------------|-----------------------------------------------------------------------|----------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Auto (Default)**   | `auto`         | None (uses CLI logins / keys if present)                              | N/A                                                      | Order: **Grok OAuth** (`~/.grok/auth.json`) → **OpenAI Codex OAuth** (`~/.codex/auth.json`) / `OPENAI_API_KEY` → **Grok API** (`GROK_API_KEY`)                                                            |
| **Google**           | `google`       | None                                                                  | N/A                                                      | None                                                                                                                                                                                                      |
| **Bing**             | `bing`         | None                                                                  | N/A                                                      | None                                                                                                                                                                                                      |
| **302.AI**           | `302ai`       | `X302AI_API_KEY`, `X302AI_MODEL`                                         | `[Your Key]`, `Gemma-7B` | See [302.AI](https://share.302.ai/tqTWfD)                                                                                                                                                   |
| **OpenAI**           | `openai`       | `OPENAI_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_REASONING_EFFORT`, `OPENAI_STOP_TOKENS`, `OPENAI_MAX_TOKENS` | `https://api.openai.com/v1`, `[Your Key]`, `gpt-5.6-luna`, `medium`, ` `, `-1` | GPT‑5.6 family: `gpt-5.6-luna` / `terra` / `sol`. Reasoning: `low`–`max`. See [OpenAI](https://platform.openai.com/docs/overview) |
| **OpenAI Codex (OAuth)** | `openai-codex` | optional `OPENAI_CODEX_MODEL`, `OPENAI_CODEX_REASONING_EFFORT`, `OPENAI_CODEX_AUTH_PATH` | `gpt-5.6-luna`, `medium`, `~/.codex/auth.json` | ChatGPT subscription via Codex CLI (`codex login`). Models: Luna/Terra/Sol. Reasoning effort sent as Responses `reasoning.effort`. |
...
|**Grok**|`grok`| `GROK_API_KEY` (optional), `GROK_MODEL`, `GROK_BASE_URL` (optional), `GROK_AUTH_PATH` (optional) | OAuth via `~/.grok/auth.json` if no API key; `grok-4.5`; `https://api.x.ai/v1` |See [Grok](https://docs.x.ai/docs/overview) / Grok Build (`grok login`). **Note:** When using custom proxy, ensure `GROK_BASE_URL` ends with `/v1`.|
