...
| **OpenAI Codex (OAuth)** | `openai-codex` | optional `OPENAI_CODEX_MODEL`, `OPENAI_CODEX_AUTH_PATH` | `gpt-5.4`, `~/.codex/auth.json` | ChatGPT subscription via Codex CLI (`codex login`). Uses Codex Responses API. |
...
|**Grok**|`grok`| `GROK_API_KEY` (optional), `GROK_MODEL`, `GROK_BASE_URL` (optional), `GROK_AUTH_PATH` (optional) | OAuth via `~/.grok/auth.json` if no API key; `grok-4.20-0309-non-reasoning`; `https://api.x.ai/v1` |See [Grok](https://docs.x.ai/docs/overview) / Grok Build (`grok login`). **Note:** When using custom proxy, ensure `GROK_BASE_URL` ends with `/v1`.|
